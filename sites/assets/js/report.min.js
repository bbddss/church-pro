var Rollup = (function (exports) {
	...
	class ColumnManager {
	    ...
	    bindDropdown() {
	        ...
	        var $activeDropdown;
	        var activeClass = 'dt-dropdown--active';
	        var toggleClass = '.dt-dropdown__toggle';

	        $.on(this.header, 'click', toggleClass, function (e, $button) {
	            var $dropdown = $.closest('.dt-dropdown', $button);
	        ...
	        var deactivateDropdownOnBodyClick = function (e) {
	            var selector = [toggleClass, toggleClass + ' *'].join(',');
	        ...
	        $.on(this.header, 'click', '.dt-dropdown__list-item', function (e, $item) {
	            var $col = $.closest('.dt-cell', $item);
	            var ref = $.data($item);
	            var index = ref.index;
	            var ref$1 = $.data($col);
	            var colIndex = ref$1.colIndex;
	    ...
	    bindFilter() {
	        ...
	        var handler = function (e) {
	            this$1.$filterCell = $.closest('.dt-cell', e.target);
	            var ref = $.data(this$1.$filterCell);
	            var colIndex = ref.colIndex;
	            var keyword = e.target.value;
	            this$1.appliedFilters[colIndex] = keyword;
	            this$1.applyFilter(this$1.appliedFilters);
	        };
	        ...
	        contains: function contains(keyword, cells) {
	            return cells
	                .filter(function (cell) {
	                    var hay = String(stripHTML(cell.html || '') || cell.content || '').toLowerCase();
	...
	function guessFilter(keyword) {
	    ...
	    if (keyword.split(':').length === 2 && keyword.split(':').every(isNumber)) {
	        return {
	            type: 'range',
	            text: keyword.split(':').map(function (v) { return v.trim(); }).map(Number)
	        };
	    }
			...
			frappe.realtime.on("report_generated", function (data) {
				if(data.report_name) {
					...
					if(data.name == this$1.prepared_report_doc_name) {
						...
						var alert_message = "Report " + (this$1.report_name) + " generated.\n\t\t\t\t\t\t<a target='_blank' href=\"#query-report/" + (this$1.report_name) + "/?prepared_report_name=" + (data.name) + "\">View</a>";
