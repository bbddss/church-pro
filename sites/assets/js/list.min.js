(function () {
	...
	frappe.templates['edit_filter'] = '<div class="filter-box">  <div class="list_filter row">   <div class="fieldname-select-area col-sm-4 form-group ui-front"></div>   <div class="col-sm-2 form-group">    <select class="condition form-control">     <option value="=">{%= __("Equals") %}</option>     <option value="like">{%= __("Like") %}</option>     <option value="in">{%= __("In") %}</option>     <option value="not in">{%= __("Not In") %}</option>     <option value="!=">{%= __("Not Equals") %}</option>     <option value="not like">{%= __("Not Like") %}</option>     <option value=">">{%= __(">") %}</option>     <option value="<">{%= __("<") %}</option>     <option value=">=">{%= __(">=") %}</option>     <option value="<=">{%= __("<=") %}</option>     <option value="Between">{%= __("Between") %}</option>     <option value="descendants of">{%= __("Descendants Of") %}</option>     <option value="not descendants of">{%= __("Not Descendants Of") %}</option>     <option value="ancestors of">{%= __("Ancestors Of") %}</option>     <option value="not ancestors of">{%= __("Not Ancestors Of") %}</option>    </select>   </div>   <div class="col-sm-6 col-xs-12">    <div class="filter-field pull-left" style="width: calc(100% - 70px)"></div>    <div class="filter-actions pull-left">     <a class="set-filter-and-run btn btn-sm btn-primary pull-left">      <i class=" fa fa-check visible-xs"></i>      <span class="hidden-xs">{%= __("Apply") %}</span></a>     <a class="small grey remove-filter pull-left">      <i class="octicon octicon-trashcan visible-xs"></i>      <span class="hidden-xs">{%= __("Remove") %}</span></a>    </div>    <div class="clearfix"></div>   </div>  </div> </div> ';
	...
	class FilterArea {
		...
		make_standard_filters() {
			...
			if (fields.length > 3) {
				fields = fields.map(function (df, i) {
					if (i >= 3) {
	...
	frappe.views.ListView = class ListView extends frappe.views.BaseList {
		...
		get_menu_items() {
			...
			if (frappe.user_roles.includes('System Manager')) {
				...
				items.push({
					label: __('Customize'),
					action: function () { return frappe.set_route('Form', 'Customize Form', {
						doc_type: doctype
					}); },
					standard: true
				});
		...
		function new_kanban_dialog() {
			...
			dialog = new frappe.ui.Dialog({
				...
				primary_action: function primary_action(values) {
					...
					var field_name = custom_column ? 'kanban_column' : values.field_name;
	...
	frappe.views.FileView = class FileView extends frappe.views.ListView {
		...
		set_breadcrumbs() {
			...
			var last_folder_route = '#' + route.join('/');
